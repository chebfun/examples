...
%%
% In this example we discuss numerical analytic continuation into the complex
% plane using the `ratinterp` command in order to approximate singularities of
% solutions to the Lorenz equations.

%% The Lorenz system in real time
% The Lorenz system is a system of ODEs first studied by Edward Lorenz in the
% 1960s as a simplified model of convection rolls in the upper atmosphere [2].
...
% In "Complex singularities and the Lorenz attractor" [3], Viswanath and
% Sahutoglu make the point that although the Lorenz attractor is a well known
% example in applied mathematics, relatively little is known about its
% mathematical analysis. It is suggested that a natural way to view the
% analysis is as a problem in analytic function theory, considering time as a
% complex variable.
...
%
% where $J$ is an integer, $P_j$ is a polynomial and $b$ is a complex number
% with $|b| = 1$.
...
%%
% Asymptotically, the singularities behave like poles of orders 1, 2 and 2 for
% $x$, $y$ and $z$ respectively, because the logarithmic terms are
% "overpowered" in the limit as $t \rightarrow t_0$. A rational approximation
% is therefore appropriate for computing the locations of the singularities.

%% Analytic continuation via rational interpolation
% We can compute a rational interpolant of the solution using `ratinterp`,
% which actually computes a least squares approximation to the interpolation
% problem, removing poles that are likely to be spurious [1, Sec. 2]. The
% following command computes rational fits of types $(m, n) = (226,40)$,
% $(248,40)$ and $(242,40)$, where $m$ is the degree of the numerator and $n$
...
%%
% Here we have taken $m$ to be half the degree of the chebfun $u$ and $n$ to
% be a number greater than the number of singularities we expect to find. We
% don't actually expect to find 40 singularities, because `ratinterp` will
% reduce the degree of the numerator automatically to avoid extra unwanted
% poles.

%%
% Here we have a contour plot of the rational approximants $r_1$, $r_2$,
% $r_3$, computed by `ratinterp` with the computed locations of singularities
...
% complex singularities. On the other hand, it has been proven that the
% imaginary parts of any singularity of the Lorenz attractor must have
% absolute value greater than $0.037$ [3].

%%
% Note that by the Psi-series formula, each of $x$, $y$ and $z$ should have
% the same singularities. Let's compare our computed values to obtain a lower
% bound on the error. The first column in the following contains the computed
% locations of the singularities of $x$, the second those of $y$ and the third
% those of $z$; the final column is the maximum separation in absolute value
...
% solve the Lorenz system, find rational approximations, then compute the
% location of their poles by finding the roots of the denominator. Considering
% this, the agreement we see here is not bad, although it would be desirable
% to have better.
...
%%
% There are many unwanted, spurious poles. They are paired with roots of the
% numerator, but only up to machine precision, so they don't actually cancel
% each other out. These are sometimes called _Froissart doublets_, for the
% astrophysicist Marcel Froissart.
subplot(1,1,1)
