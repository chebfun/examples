...
%
% where $\psi(x)$ is the wavefunction, $\hbar$ is Planck's constant divided
% by $2\pi$, $m$ is an effective mass, $E$ is an allowed energy, and $U(x)$ is
% a potential with wells representing the quantum dots. We will take $\psi=0$
% far from the wells to supply boundary conditions.

%%
% Here are some dimensional parameters corresponding to physical experiments.
% Space is measured in nanometers and energy is in electron volts.
...
%%
% From the well parameters we will find the boundaries of each well, then add
% endpoints far out to each side.
...
% Rather than getting one isolated wavefunction "bump" per well, we see that
% the wells influence one another strongly.
...
%%
% In practice, though, the potential wells cannot be fabricated identically.
% A brief experiment shows that the delocalization deteriorates even with
...
%    intermediate states with realistic quantum dots." _Journal
%    of Applied Physics_, 111,
