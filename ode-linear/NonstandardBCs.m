...
% became possible to solve ODEs in Chebfun with more exotic constraints than
...
% domain.
%
% This example describes how to solve problems of this kind. All the ODEs here
% are linear, but the approach extends to nonlinear problems too.
%
...
%
% An equivalent way to state the second condition is that the definite
...
%%
% However, the second condition applies on the function over the whole domain.
% We use the field `.bc` to enforce the condition
N.bc = @(x,u) sum(u);

%%
% Note that here N.bc is an anonymous function. Chebfun interprets this to be
% a condition that applies on the solution over the interior of the domain
% (compared to, e.g., `N.bc = 1` or `N.bc = 'dirichlet'`, which automatically
% get transformed into conditions on both boundaries). Further, note that the
% anonymous function can take `x`, the independent variable on the domain as
% an argument, allowing you to introduce weighting functions in problems.
...
%%
% Finally, we could also introduce some weighting, for example, we could look
% for a solution which is orthogonal to $\sin(4\pi x)$:
N.bc = @(x,u) sum(sin(4*pi*x).*u);
u = N\1; plot(u,LW,1.6), grid on
%%
% Again, we obtained an accurate solution (to evaluate the interior point
% condition, we need to create a chebfun for $x$):
disp(['Residual of differential equation: ', num2str(norm(N(u)-1))])
disp(['Residual of left BC:               ',  num2str(abs(u(-1)-1))])
x = chebfun('x');
...
% $$ u'(0) = 1, $$
%
% we need to use a little trick in order to be able to evaluate the derivative
% at an interior point. MATLAB doesn't allow the stacked parentheses we would
% need for something like `@(x,u) diff(u)(0)`, so instead this is achived
% using the `feval` method:
