...
%% Introduction
% Rotating functions defined on the sphere have applications in many fields,
...
subplot(1,2,1)
plot( f ), title('Original','fontsize',16), axis off
subplot(1,2,2)
plot( g ), title('Rotated','fontsize',16), axis off

%% 
% We would like to point out that the |rotate| command requires a
...
% f and rotate(f) are represented in the same coordinate system, allowing for 
% convenient algebraic manipulations. 
h = f + g;    
plot( h )
...
subplot(2,2,2)
g = rotate( f, pi/4, 0, 0);
plot( g ), title('Rotated \phi=\pi/4','fontsize',16), axis off
subplot(2,2,3)
g = rotate( f, 0, pi/4, 0);
plot( g ), title('Rotated \theta=\pi/4','fontsize',16), axis off
subplot(2,2,4)
g = rotate( f, pi/4, 0, pi/4);
plot( g ), title('Rotated \phi=\psi=\pi/4','fontsize',16), axis off
...
plot( f ), title('Random function','fontsize',16), axis off
...
plot( f ), title('Rotated random function','fontsize',16), axis off
...
%%
% Similar to the rotated functions in [5], we find that the required rank
...
% is a rotation that increases the rank by about $$5$$: 
...
% rotated, we consider the Gaussian function restricted to the sphere.  
...
plot(alp,rk,'x-','LineWidth',2), xlabel('Rotation angle','fontsize',16)
ylabel('Rank'), title('Rank of the rotation of a Gaussian','fontsize',16)
...
% the Gaussian reduces to $$\exp(-20(1\pm z))$$ at north and south poles,
% respectively.
...
% May 2016. 
%%
% [5] L. N. Trefethen, "low rank compression of square and round pegs",
% Chebfun Example, June 2016. 