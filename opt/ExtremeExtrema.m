...
hp = diff(h);
extrema = roots(hp);
plot(extrema,h(extrema),'.k','markersize',8)
...
% Chebfun looks for zeros of the derivative. In some cases those are points
% where the derivative is continuous and passes through zero. In others, like
% the leftmost black dot in the plot above, they are points where the
