...
% Since the introduction of Chebfun2 in 2013, this
% would normally done with Chebfun2, but this example, 
% originally written in 2010, uses 1D Chebfun to do the job.
% The "Rosenbrock revisited" Example treats the
% same problem more properly, and
% more efficiently, with Chebfun2 [3].

%%
% The example we consider is the famous and challenging
% "Rosenbrock function":
...
% is 0, taken at x=y=1.  In 1D Chebfun, we can find this
...
% actually necessary in this case, but it would be necessary for more
% general functions f(x,y) having several local extrema, because then
% the function fminx might not be smooth.
...
% lack of smoothness and the consequent need for edge detection,
% this computation takes a little while:
tic
