...
% parameter, and we iterate with the matrix defined like this:
% $$ G = M^{-1} N, \qquad M = D + \omega L, \quad N = (1-\omega)D- \omega U. $$
% In MATLAB, that's
G = @(om) (D+om*L)\((1-om)*D-om*U);
rho = @(om) max(abs(eig(G(om))));

%%
% Analysis of the SOR iteration was carried out by Frankel [1] and generalized
% by Young [4]; see also [3].  Details are given in innumerable books, such as
% Golub and Van Loan [2]. Supposing we didn't know the theory, Chebfun would
...
% iteration, IMA Journal of Numerical Analysis 8 (1988), 273-279.
%
% [4] D. M. Young, Iterative Methods for Solving Partial Difference
