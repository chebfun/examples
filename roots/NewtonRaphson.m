...
% vicinity of a suspected root to find successively better approximations to
% the root, using the formula
...
%%
% If we try to locate the leftmost root by Newton's method, we need to pick an
% initial guess, for example $-3$.
...
%%
% In the above plot, the solid black dots are the successive approximations of
% the root, while the circles are their projections on the curve, from which
% the Newton's method locates the next approximation along the tangent (black
% dash-dot lines). The following table tells us with no surprise that Newton's
