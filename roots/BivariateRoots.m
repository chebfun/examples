...
%%
% Suppose $f(x,y)$ and $g(x,y)$ are bivariate polynomials and we are
% interested in finding the common roots $(x_\ast,y_\ast)$ for which
% $f(x_\ast,y_\ast)=g(x_\ast,y_\ast)=0$. This problem arises for example when
% locating the critical points of a surface $z = h(x,y)$ since the critical
% points satisfy
% $$ f(x,y) = \frac{\partial z}{\partial x} = 0 , \quad g(x,y) =
% \frac{\partial z}{\partial y}= 0. $$

%%
% One way to do this [2, 3] is to form the Bezout matrix. Given two
% polynomials $f(x)$ and $g(x)$, the Bezoutian is defined as the bivariate
% polynomial
% $$ B(x,z) = \frac{f(x)g(z)-g(x)f(z)}{x-z}. $$
% It is often helpful to expand $B(x,z)$ into a bivariate Chebyshev expansion [3],
% $$ B(x,y) = \sum_{i,j} a_{i,j}T_{i-1}(x)T_{j-1}(y), $$
% where $T_i(x)$ is the ith Chebyshev polynomial (other polynomial bases can
% be used). The matrix $A = (a_{ij})$ is called the (generalised) Bezout
% matrix because it is generated from the Bezoutian.

%%
% A well known fact [1] is that the matrix $A$ is singular if and only if the
% two polynomials $p(x)$ and $q(x)$ have a common root. We take advantage of
% this property in the following way. Regard $f(x,y)$ and $g(x,y)$ as
% univariate polynomials in $x$, with coefficients in $y$. Then we can form
% the Bezout matrix $A(y)$, whose elements are now seen as functions of $y$. A
% value $y_\ast$ such that $A(y_\ast)$ is singular indicates $f(x,y_\ast)$ and
% $g(x,y_\ast)$ have at least one common root $x_\ast$ and hence,
% $f(x_\ast,y_\ast)=g(x_\ast,y_\ast)=0$.

%%
% Let's illustrate bivariate rootfinding with an example. Suppose we want to
% find the intersections of the two curves $u_1(x) + iv_1(x)$ and
% $u_2(y) + iv_2(y)$.  We can write this as the following bivariate
% rootfinding problem:
%
% $$ f(x,y) = u_1(x) - u_2(y),\quad  g(x,y) = v_1(x) - v_2(y). $$
%
% (The curves we selected were obtained by modifying curves from Example
% [complex/RoucheTheorem](../complex/RoucheTheorem.html)).
...
% valued chebfuns. To keep the computation of a manageable size we need to set
% the maximum degree $n$ to a few dozen, as the complexity of what follows is
% $O(n^6)$.
...
% For convenience we first represent the bivariate polynomials $f(x,y)$ and
% $g(x,y)$ as matrices of coefficients in their bivariate expansions, as we
% did above.
...
% $x$) by using the code DLP.m, available from [4]. The code forms a
% linearization $\lambda X+Y$ for a given matrix polynomial, and in [4] it is
% shown that $X$ is a Bezout matrix. We use this to construct the Bezout
% matrix $A$ corresponding to the Bezoutian of $f(x,y)$ and $g(x,y)$.
...
%%
% $A(y)$ is the Bezout matrix (in the code $A(:,:,i)$ is the $i$-th leading
% coefficient matrix in the Chebyshev basis), and we want to find the values
% of $y$ such that $A(y)$ is singular. This reduces to a polynomial eigenvalue
% problem of size $n\times n$ and degree $n$.

%%
% The standard way to solve a polynomial eigenvalue problem is via
% linearization followed by the solution of the corresponding generalized
% eigenvalue problem.
%
...
% This forms a generalized eigenvalue problem $\lambda X+Y$ whose eigenvalues
% match those of $B(y)$.
...
% intersecting points, that is `[u2(yval), v2(yval)]`. To get the $x$-values,
% we use the important but not so well known fact that if the Bezoutian has a
% null space, then it is of the Vandermonde form $[T_{n-1}(x),\ldots,
% T_2(x),T_1(x),T_0(x)]^T$. We extract the "correct" computed roots by finding
% the solution in the region of interest $x,y\in [-1,1]$.
