...
%% Try converting to ADCHEBFUNs and seeding
u = adchebfun(u);
v = adchebfun(v);
u = seed(u, 1, 2);
v = seed(v, 2, 2);
...
% separately.
r = F(u,v) % Evaluating on ADCHEBFUNs
r1 = getElement(r, 1);
r2 = getElement(r, 2);
r1jac = r1.jacobian;
r2jac = r2.jacobian;
...
% linode = @(t,y) [diff(y{1}); diff(y{2})] - (Ju*y{1} + Jv*y{2});
linode =  @(t,y) [diff(y{1}) - Ju{1}*y{1} - Jv{1}*y{2}; ...
    diff(y{2}) - Ju{2}*y{1} - Jv{2}*y{2}];
A = chebop(linode, [0,T] );
A.init = [ 0*u.func; 0*v.func ];

%%
% Use the first column of the identity as the initial condition:
A.bc = @(t,y) [y{1}(0)-1; y{2}(0)];
Y1 = A\0;
