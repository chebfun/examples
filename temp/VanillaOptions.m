...
% formulas, leaving the five market/contract parameters as input variables. The sixth
% input, $W$, is set to 1 for a call option and -1 for a put option.
...
%
% In practice it is often the case that one cannot trade the instruments
% with the exact desired parameters. Chebfun allows us to quickly calculate
% the impact of a hedge based on an imperfect put-call parity relation. For 
% example, consider the case of selling an at-the-money 6-months call option, which could
% incur in great losses if the asset increase too much in value. The
% risk can be hedged by buying the underling asset (which will protect us
% if the price goes up) and buying a put (which will protect us if the
% price goes down), financed with a loan for an ammount $K$.

call = chebfun(@(s) vanilla(s,100,.5,vol,r,+1), [0 300]);
asset = chebfun(@(s) s, [0 300]);
loan = K;
%%
% We consider three strategies based on the choice of the put: one with the same paremeters as the
% call (blue), one with a higher strike and shorter maturity (red), and one with lower
% strike, longer maturity and slightly larger notional of 1.03 instead of 1
% (black). We set these strikes/maturities so the hedge is at par when
% $S=100$.
r = 0;
...
% depicted in blue in the first figure of this example. The _time value_ is the difference
% between the price and the implicit value and it measures
...
%%
% By increasing the rate from 0 to 1.5% the behaviour of the vanilla
% options has changed completely, particularly for the put. We highlight a couple of interesting 
% features. First, notice how for the call option the time value decays to
% zero to the left of the strike but reaches a plateau to the right of it,
% at the level $K-K\exp(-rT)$. Second, notice how for the put option the 
% time value is negative to the left of the
% strike. This means that if the underlying asset was trading at a very low
% level, deep in-the-money, it would be more profitable to exercise immediately the option
% rather than holding to it. This considerations is irrelevant for European
% options, which cannot be exercised before the expiry date, but crucial
% for _American options_, which incorporate an early exercise feature.
%
...
%%
% On the other hand the time value of a call option remains positive with
% a non-zero $r$, which means that for a call an early exercise would never be more profitable than
% holding the option until maturity. 
...
%% References
%
% [1] S. Natenberg, _Option Volatility & Pricing: Advanced Trading 
% Strategies and Techniques_, McGraw-Hill Professional, 2nd edition, 1994.
%
% [2] J. Hull, _Options, Futures and Other Derivatives_, Pearson Education,
% 9th edition, 2014.
