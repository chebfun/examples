...
% Consider a trajectory $\boldmath{x}(t)$ in phase space and a nearby
% trajectory $\boldmath{x}(t) + \delta(t)$, where $\delta(t)$ is a
...
%%
% The reason we've used the word "maximal" is that a dynamical system can have
% more than one Lyapunov exponent. In fact, every dynamical system has a
% spectrum of Lyapunov exponents, with one for every dimension of its phase
% space.
...
%%
% Let's compute the maximal Lyapunov exponent of the Lorenz system.
% We will solve for two nearby trajectories on a reasonably large
% time interval, say $t\in[0,30]$.
...
%%
% Now solve for two trajectories that have a small initial separation.

ep = 1e-9;
N.lbc = @(x,y,z) [x+1; y+4; z-24];
[x1,y1,z1] = deal(N\0);    % Components of 1st trajectory
N.lbc = @(x,y,z) [x+1; y+4; z-24-ep];
[x2,y2,z2] = deal(N\0);    % Components of 2nd trajectory

%%
% Now find the distance between trajectories using the distance formula. We'll
% plot the distance using a log scale on the y-axis.
...
%%
% The log of the distance between trajectories approximates a straight line
% with positive slope, so it seems the Lorenz system has a positive Lyapunov
% exponent.

%%
% Notice, however, that the positive slope only holds up for the first 20 time
...
% `d`. We'll only use the first 20 time units, the range where the separation
...
hold on
x = chebfun('x', [0 dom(2)]);
semilogy(.5e-9 * exp(slope*x), 'k--')
legend('dist(x_1, x_2)', sprintf('exp(%1.2f x)', slope), ...
    'location', 'northwest')

%%
% This approximation isn't bad at all -- the maximal Lyapunov exponent for the
% Lorenz system is known to be about $0.906$. To calculate it accurately we'd
% need to average over many trajectories.
