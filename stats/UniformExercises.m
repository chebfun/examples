...
% concerning various probability distributions.  In this Example we use
% Chebfun to solve two problems involving the uniform distribution from the
% textbook [1]. The domain is a finite interval. Other similar Examples look
% at problems from the same book involving the normal, beta, exponential,
% gamma, Rayleigh, and Maxwell distributions.

%%
% Like most textbooks, [1] emphasizes problems that can be solved on paper and
% don't need numerical tools such as Chebfun. As soon as one varies the
% problem a little, however, numerical solutions often become necessary. Thus
% first we solve the problems as written, and then we show an application.
...
% <blockquote>
% Suppose $X$ is a continuous random variable with uniform distribution having
% mean $1$ and variance $4/3$. What is $P[X<0]$?
% </blockquote>
...
%%
% Then the mean is $(a+b)/2$ and the variance is $(b-a)^2/12$. We want to
% solve for $a$ and $b$ given the stated conditions on the mean and variance.
...
%%
% Let us assume that the roots are within a square of side length $10$
...
%%
% We then get two pairs of values for $a$ and $b$. Since $a < b$, we can
% easily make the right choice of the $(a,b)$ pair and discard the other one.
...
%%
% By $variance = (b-a)^2/12 = 4/3$, we can try $a$ in the interval $[-5,5]$ to
% see if we get a root for the function $g$ defined as:
...
%%
% Another Chebfun approach would be to solve for both $a$ and $b$
% using Chebfun2.
...
% rest. The wheel is partitioned into ordered sectors: red ($5^\circ$), cyan
% ($15^\circ$), yellow ($35^\circ$), green ($50^\circ$), white ($65^\circ$),
% blue ($80^\circ$), black ($110^\circ$). Each color represents a prize. When
% the wheel comes to rest, a fixed arrow on the circumference points to a
% color. The participant then receives the prize that the corresponding color
% stands for. We can see that the point on the circumference of the wheel that
% is located opposite the fixed arrow could be considered as the value of a
% random variable $X$ that is uniformly distributed over the circumference of
% the wheel.
% </blockquote>

%%
% <blockquote>
% (Q1) What is the probability of a participant receiving a prize represented
% by red or cyan?
% </blockquote>
...
%%
% <blockquote>
% (Q2) We now know that the arrow will not point to the blue region when the
% wheel comes to rest. Under this condition, what is the probability that it
% will point to neither black nor yellow?
% </blockquote>
...
%%
% So the probability of the arrow not pointing to any of yellow, black, or
% blue is:
pn = pnyb-fint(80)
