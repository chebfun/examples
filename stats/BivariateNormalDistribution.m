...
% distribution, where two normal distributions $X$
% and $Y$ are coupled together. In
...
%%
% The value above is less than 1 because of numerical error, not because
...
% probability distribution after averaging over information about $Y$.
% This can be
% calculated by just integration over $y$ with the `sum`
% command (the sum of a chebfun2 defaults to integration over $y$).
px = sum(p);
plot(px),
...
% function of $Y$? This is the called conditional probability, 
% which is associated with
% the concept of conditional probability distributions. We can use Chebfun2 to
% compute numerically the conditional probability distribution, restricting
