...
% case letter for a scalar function, $f$, and an upper case letter for a
% vector function, $F = \left(f_1,f_2\right)^T$. Vector valued functions can
% be approximated by objects of type chebfun2v. There are two standard ways to
% make a chebfun2v:
...
% Vector addition, denoted $F + G$, yields another chebfun2v and is computed
% by adding the two scalar components together. It satisfies the parallelogram
% law, which can be verified numerically, as in this example:
...
% The gradient of a chebfun2 is represented by a chebfun2v and is a vector
...
% $\mathrm{curl}(F)$ is a scalar function equal to twice the angular speed of
% a particle in the flow at each point. A particle moving in a gradient field
% has zero angular speed and hence, $\mathrm{curl}(\mathrm{grad}(f))=0$, a
% well known identity that can also be checked numerically. For example,
