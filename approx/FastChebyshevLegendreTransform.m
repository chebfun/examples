...
% Chebyshev coefficients and (2) The inverse transform, `cheb2leg`, which
...
% Chebyshev coefficients, a phenomenon discussed in [5].  Here we witness this
% disparity for the function $|x-.1|^{7/4}$:
...
% Recently, an ultraspherical spectral method was developed, which solves linear
% constant-coefficient
% ODEs in essentially $O(N)$ operations and computes the Chebyshev coefficients
% of the solution [3]. This approach easily generalises to a fast
% Legendre spectral method. Now that we have a fast transform, we can
...
%% Conclusion
% The existence of a fast and practical Chebyshev-Legendre transform has
% many implications.  Within Chebfun, it has led to major speedups
% in the commands `conv` and `polyfit`.  Further developments are in
% the pipeline.
