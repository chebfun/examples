...
%% Introduction
% Let us approximate a continuous function $f$ defined on $[-1,1]$ 
...
% The error consequently changes sign at least $n+1$ times [1]. Let
...
%%
% Notice that since Chebfun normally uses
% Chebyshev points of the second kind for
% interpolation, which include the end points $-1$ and $1$, the error curve will
% always have zeros at $-1$ and $1$.  You will get a different
% effect however if you run this example in a mode using
% Chebyshev points of the first kind rather than the second kind.
